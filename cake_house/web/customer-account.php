...
if(isset($_POST['EditForm']) && $_POST['EditForm'] == "UPDATE"){
    echo $_POST['EditForm'] ;
    $sql= "UPDATE members SET account=:account, password=:password, name=:name, phone=:phone, email=:email , birthday=:birthday, 	mobile=:mobile, gender=:gender, county=:county , district=:district, 		zipcode=:zipcode, address=:address  , 	updated_at=:updated_at where memberID=:memberID";
    $sth = $db ->prepare($sql);
